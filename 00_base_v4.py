...
# checks numbers where necessary to be more or equal to required number
def num_check(question, error, num_type, num):
    valid = False
    while not valid:
...
budget = num_check("What is your budget in dollars? ",
...
while item != "xxx":
    ...
    # get weight in grams
    weight_grams = num_check("What is the weight in grams / milliliters? ",
                             "Please enter a whole number that is 50 or more\n", int, 50)

    # get cost in dollars
    get_cost = num_check("What is the price in dollars? ",
                           "Please enter a number that is 0.5 or more\n", float, 0.5)
